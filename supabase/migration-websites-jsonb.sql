-- ==============================================================================
-- 1. Create the single multi-tenant 'websites' table
-- ==============================================================================
...
-- Policies
drop policy if exists "Allow public read access on websites" on public.websites;
drop policy if exists "Allow full access on websites" on public.websites;

create policy "Allow public read access on websites"
  on public.websites for select
  using (true);

create policy "Allow full access on websites"
  on public.websites for all
  using (true)
  with check (true);

-- ==============================================================================
-- 2. Populate complete rich data for Aurumm (All sections, collections, gems)
-- ==============================================================================
insert into public.websites (id, name, domain, content, updated_at)
values (
  'aurumm',
  'Aurumm Fine Jewellery',
  'localhost:5173',
  '{
  "sections": {
    "hero": {
  ...
  "updatedAt": "2026-09-07T06:56:21.255Z"
}'::jsonb,
  now()
)
...
-- ==============================================================================
-- 3. Safely drop the 6 old relational tables (now replaced by single JSONB table)
-- ==============================================================================
drop table if exists public.site_content cascade;
drop table if exists public.collections cascade;
drop table if exists public.custom_categories cascade;
drop table if exists public.gemstones cascade;
